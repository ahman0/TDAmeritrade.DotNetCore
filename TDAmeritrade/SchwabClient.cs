...
namespace TDAmeritrade
{
    public class SchwabClient
    {
        ...
        public SchwabClient(ITDPersistentCache cache)
        {
            _cache = cache;
        }
        ...
        public string GetSignInUrl(string consumerKey, string redirectUrl = "http://localhost/")
        {
            ...
            var path = $"https://api.schwabapi.com/v1/oauth/authorize?redirect_uri={encodedUri}&client_id={encodedKey}";
            return path;
        }
        ...
        public async Task SignIn(string consumerKey, string code, string clientSecret, string redirectUrl = "http://localhost/")
        {
            var decoded = HttpUtility.UrlDecode(code);
            var path = "https://api.schwabapi.com/v1/oauth/token";
            ...
            using (var client = new HttpClient())
            {
                ...
                using var req = new HttpRequestMessage(HttpMethod.Post, path) { Content = new FormUrlEncodedContent(dict) };
                req.Headers.Authorization = new AuthenticationHeaderValue(
                    "Basic", Convert.ToBase64String(Encoding.UTF8.GetBytes($"{consumerKey}:{clientSecret}")));
                ...
                switch (res.StatusCode)
                {
                    case HttpStatusCode.OK:
                        ...
                        AuthResult.consumer_secret = clientSecret;
                        AuthResult.redirect_url = redirectUrl;
                        _cache.Save("SchwabKey", JsonConvert.SerializeObject(AuthResult));
                        IsSignedIn = true;
                        HasConsumerKey = true;
                        OnSignedIn(true);
                        break;
        ...
        {
            AuthResult = JsonConvert.DeserializeObject<SchwabAuthResult>(_cache.Load("SchwabKey"));
            ...
            var dict = new Dictionary<string, string>
                {
                    ...
                    //{ "access_type", "" },
                    //{ "client_id", AuthResult.consumer_key },
            ...
            req.Headers.Authorization = new AuthenticationHeaderValue(
                "Basic", Convert.ToBase64String(Encoding.UTF8.GetBytes($"{AuthResult.consumer_key}:{AuthResult.consumer_secret}")));
            using (var client = new HttpClient())
            {
                var res = await client.SendAsync(req);
